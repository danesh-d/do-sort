          ...
          while (swapped) {
            ...
            for (LL i = lower_bound; i < upper_bound; ++i) {
              if (sort<T>::v[i] > sort<T>::v[i + 1]) {
                ...
                // distributed random numbers. But if the list is partially
                // sorted at the begining then it will move the lower bound so
                // in the next round, the sorting will start after the sorted
                // sequence.
                if (!swapped) {
                  tmp_lower = i;
                }
            ...
            // the process up to the last non-sorted element in the list. Also,
            // jump over all sorted elements at the begining of teh array.
